...
arthropods_t6 <- arthropods[arthropods$t == 6, ]
# - Explicit mapping:
(cou_t6 <- count(arthropods_t6, mapping(x = x, y = y, t = t, i = i)))
# - Total implicit mapping:
cou_t6_bis <- count(arthropods_t6)
# - Partial implicit mapping:
cou_t6_ter <- count(arthropods_t6, mapping(i = i))
all(identical(cou_t6, cou_t6_bis), identical(cou_t6, cou_t6_ter))
...
cou_t6_distr <- fit_two_distr(cou_t6)
summary(cou_t6_distr)

inc9_t1_distr <- fit_two_distr(inc9_t1)
summary(inc9_t1_distr)

plot(cou_t6_distr, breaks = 17)
plot(inc9_t1_distr)
```
...
```{r sadie_test, fig.height = 5, fig.show = "hold"}
set.seed(123)
cou_t6_m <- remap(cou_t6, mapping(x = xm, y = ym))
res  <- sadie(cou_t6_m)
summary(res)
plot(res)
