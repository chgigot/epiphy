...
#' @export
#------------------------------------------------------------------------------#
summary.fisher <- function(object, ..., test = c("chisq", "calpha", "z")) {
    test <- match.arg(test)
    switch (test,
        "chisq"  = chisq.test(object),
        "calpha" = calpha.test(object),
        "z"      = z.test(object)
    )
}
...
#' }
#'
#' @seealso \code{\link{fisher}}, \code{\link{morisita}}
#'
...
lloyd.default <- function(x, type = c("patchiness", "mean-crowding")) {
    ...
    switch (type,
        ...
        "patchiness"    = {
            structure(list(index = mc / m), class = c("lloyd", "agg_index"))
        }
    )
}
...
morisita.default <- function(x, flavor = c("count", "incidence"), n = NULL, ...) {
    ...
    structure(list(index = Idelta), class = c("morisita", "agg_index"))
}
